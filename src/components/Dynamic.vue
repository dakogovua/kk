<template>
    <div>
        <div class="searchContainer">
            <input class="searchBox" v-model="search" type="search" name="search" placeholder="Search..." @keyup.enter="emitToSearch">
            <input type="submit" value="Search" class="searchButton"  @click.prevent="emitToSearch">
        </div>
        <Search :searchProp="search" ref="searchComponent" />
        <!--{{ $options.name }}<br>-->
    ...
    export default {
        ...
        methods: {
            emitToSearch () {
                ...
                this.$refs.searchComponent.action();
            },
          getSarWarsData(){
              this.loading = true
              this.$axios
